...
They can like the message, bookmark it, reply to it or branch it.
...
Message branching is covered in section \ref{sec.branching}.
...
However, it also broke relations between messages and ideas, which are made through replies and branches.
...
Basically, you can create an idea by branching another idea or a message.
...
Clicking on the branch symbol in the Ideas Overview will bring to the idea tree navigation.
We can also get there by clicking on the number of branches from the view of an idea.

\begin{figure}[!htb]
    \centering
    \begin{subfigure}[t]{.32\textwidth}
        \includegraphics[width=\textwidth]{images/branches_ideasOverview.png}
        \caption{The ideas that are branches or that have been branched are noticeable by a specific symbol symbol on the right.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{.32\textwidth}
...
Branching a message means creating a new idea.
Therefore, we could say that a message that has been branched is of great importance.
...
This is why branches only have a factor 1 in the computation of a message weight.
...
Then she tried to click on the \emph{Share your idea} button, but it was disabled as she did not write anything in the idea description, which is mandatory.
...
What we did to improve the user experience is to make the text field error message visible not only after having clicked once on the field and leaving it empty, but also when clicking on the \emph{Share your idea} button.
...
When she forked the idea and noticed the result, she said she was confused as she was rather expecting the text she wrote for the forked idea to be a new message within the original idea and not another idea.
...
After that she tried to add a message in the discussion but noticed that she could not when she saw the message saying that she has to complete her profile first.
...
To do so, on the profile tab of the person view, we added a message in bold text stating that “\emph{to complete your profile, enter your first and last name}”.
...
Then she looked at her mock profile, and in the bookmark section, she first understood that this was some kind of direct messages between users of Flok.
After thinking by herself about it, she finally got that there were ideas or messages from ideas that a user can bookmark to have them in one place.
This anyway made us realize that it was not immediately clear.
The messages notably, were just consisting of their text content and the author avatar.
We now have changed that so that they also feature the name of the idea they are from.
...
However, this test user did not realize neither that he could drag and drop ideas, even with the message “\emph{Create an idea or drag an idea here to change its status}” visible in the empty columns.
...
After that, he went to complete his profile as suggested in the non-intrusive message at the top of the ideas overview, appearing after having created a first idea.
This comforted us in the usefulness of this feature.

Only then he went to see the idea he created.
From there he clicked on the “\emph{Start the conversation}” button and added messages.
He tried the actions buttons below the messages.
...
This made us add a small notification showing when bookmarking a message or an idea.
It also contains a button that the user can click to go see his bookmarks.

When he clicked on the fork (previous name of the branch feature) button of a message, and created a new idea, he was a bit confused as he expected the forked idea to be just inside the message and not as a whole new idea.
This comforted us that the changes we brought after the first user test, by renaming the feature and giving more explanations in the form to branch an idea, are indeed needed.

After creating other ideas and adding new messages, he said he was expecting the ideas to be ordered according to recent activity.
...
To prevent this reaction, we added a message counter in the idea main view

Then he visited the recap page of an idea and wasn't really sure of how useful it would be for him.
He said he prefers having the message in chronological order.
...
We realized that the banner suggest to complete the profile, but doesn't say why it would be good to do it.
Therefore we added that it is necessary to do to be able to add comments.

She noticed that ideas can be dragged thanks to the visual changes we brought since the previous test.
...
To fix that we made the whole box draggable again, but kept the drag handle as it could be a visual cue for other users.

Then, in the idea view, she saw the message counter and wanted to see the discussion by clicking on the counter, which was not working.
We added the message counter after the previous user test and it didn't seem to have any issue visually, but we didn't really test the usability in this short period of time.
...
Then she went to the discussion of an idea and tested the different actions that can be done with one message.
...
However, she got really confused with the filters to show or hide Ideas, comments, images or documents.
It was not clear for her when they were activated or not.
To fix that we changed the design by making a filter toggle less visible when deactivated, and by adding colors when it is activated.

She liked the achievements section of her profile, but wasn't sure of the difference between the number of ideas followed and the number of ideas she took part in.
The latter counts the ideas where she posted at least one comment.
The simple fact that we feel the need to explain it here means that it requires clarification and that is why we simply changed it to “\emph{the number of ideas in which you wrote at least one message}”.

She said repetitively during the test that she likes the simplicity of the interface.
In the end, she added that this simplicity helps to understand some of the concepts that are a bit complicated.
